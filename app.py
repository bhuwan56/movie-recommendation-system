...
# CONFIG
# =============================
API_BASE = "https://movie-recommendation-system-2-39c8.onrender.com/" or "https://movie-rec-466x.onrender.com"  # fallback removed, just use main API
...
.card { border: 1px solid rgba(0,0,0,0.08); border-radius: 16px; padding: 14px; background: transparent; }
.card img:hover { transform: scale(1.03); transition: transform 0.2s; }
</style>
""",
...
def goto_home():
    ...
    # CLEAR or SET params using the new dictionary syntax
...
def goto_details(tmdb_id: int):
    ...
    # NEW SYNTAX: Assign values directly to the dictionary
    st.query_params["view"] = "details"
    st.query_params["id"] = tmdb_id
    
    # st.rerun() is the modern replacement for st.experimental_rerun()
    st.rerun()
...
def poster_grid(cards, cols=6, key_prefix="grid"):
    ...
    for r in range(rows):
        ...
        for c in range(cols):
            ...
            with colset[c]:
                if poster:
                    # Calculate dynamic width based on column width
                    col_width = int(st.session_state.get("poster_width", 200))
...
# ==========================================================
    ...
    with left:
        ...
        if data.get("poster_url"):
            st.image(data["poster_url"], width=300)  # replaced use_column_width
        else:
            st.write("🖼️ No poster")
    ...
    if data.get("backdrop_url"):
        ...
        st.image(data["backdrop_url"], width=700)  # replaced use_column_width
